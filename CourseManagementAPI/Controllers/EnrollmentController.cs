...
namespace CourseManagementAPI.Controllers
{
    ...
    public class EnrollmentController : ControllerBase
    {
        ...
        public EnrollmentController(CMSdbContext context, IMappingService mappingService)
        {
            _context = context;
        ...
        public async Task<ActionResult<EnrollmentDto>> EnrollUserInCourse([FromBody] CreateEnrollmentDto createEnrollmentDto)
        {
            ...
            if (userRole != "Admin" && userId != createEnrollmentDto.UserId)
                return Forbid("You can only enroll yourself");

            // Check if user exists
            var userExists = await _context.Users.AnyAsync(u => u.Id == createEnrollmentDto.UserId && !u.IsDeleted);
            if (!userExists)
                return BadRequest($"User with id {createEnrollmentDto.UserId} not found");

            // Check if course exists
            var courseExists = await _context.Courses.AnyAsync(c => c.Id == createEnrollmentDto.CourseId && !c.IsDeleted);
            if (!courseExists)
                return BadRequest($"Course with id {createEnrollmentDto.CourseId} not found");

            // Check if user is already enrolled
            var alreadyEnrolled = await _context.Enrollments
                .AnyAsync(e => e.UserId == createEnrollmentDto.UserId && e.CourseId == createEnrollmentDto.CourseId);
            if (alreadyEnrolled)
            ...
            return CreatedAtAction(nameof(GetEnrollmentById), new { id = enrollment.Id }, _mappingService.MapToEnrollmentDto(enrollment));
        }
        ...
        public async Task<ActionResult<IEnumerable<EnrollmentDto>>> GetAllEnrollments()
        {
            var enrollments = await _context.Enrollments
                ...
                .Include(e => e.Course)
                .ToListAsync();
            return Ok(enrollments.Select(_mappingService.MapToEnrollmentDto).ToList());
        }
        ...
        public async Task<ActionResult<EnrollmentDto>> GetEnrollmentById(int id)
        {
            ...
            return Ok(_mappingService.MapToEnrollmentDto(enrollment));
        }
